...
def test_single_filter(collection):
    query = collection.select(["title"]).filter("title").contains("Robots")
    ...
    assert "SELECT title" in captured_output.getvalue()
    assert "FROM books" in captured_output.getvalue()
    assert "WHERE title LIKE '%Robots%'" in captured_output.getvalue()
